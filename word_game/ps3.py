...
def get_word_score(word, n):
    ...
    letter_score = sum([SCRABBLE_LETTER_VALUES.get(c, 0) for c in word.lower()])
    wordlen = len(word)
    length_koef = max(1, 7 * wordlen - 3 * (n - wordlen))
    return length_koef * letter_score
...
def update_hand(hand, word):
    ...
    new_hand = hand.copy()
    for c in word_freq:
        new_hand[c] = max(0, hand.get(c, 0) - word_freq.get(c, 0))
    return {c: new_hand[c] for c in new_hand if new_hand[c]}
...
def is_valid_word(word, hand, word_list):
    ...
    if '*' in lword:
        for c in VOWELS:
            if lword.replace('*', c) in word_list:
                word_is_valid = True
                break
    else:
        word_is_valid = lword in word_list

    word_freq = get_frequency_dict(lword)
    for c in word_freq:
        if word_freq.get(c, 0) > hand.get(c, 0):
            word_is_valid = False
            break
